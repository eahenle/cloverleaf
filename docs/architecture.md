...
Codex runs server-side with the SDK's read-only sandbox preset. It can read project context supplied in a request and return complete-file proposals, but only Cloverleaf's explicit, confirmed apply flow can write those proposals. Each assistant POST has a short-lived, request-scoped WebSocket: FastAPI translates selected SDK turn/item/retry events into stable non-sensitive phases and emits a heartbeat during quiet periods while the ordinary JSON request retains normal success and error semantics. Raw reasoning, command text, and tool arguments remain server-side.
